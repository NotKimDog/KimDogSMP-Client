...
if ($versionLine) {
    ...
    Write-Host "❌ Could not find version in gradle.properties!" -ForegroundColor Red
    exit 1
}
...
if (Test-Path $javaFile) {
    ...
    Write-Host "  ✅ Updated Kimdog_smp.java" -ForegroundColor Green
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "⚠️  Push to main failed, but continuing..." -ForegroundColor Yellow
}
...
Write-Host "🏷️  Git tag created: v$newVersion" -ForegroundColor Cyan
